...
def fetch_new_messages(last_known_id=None):
    ...
    while len(all_messages) < MAX_MESSAGES_PER_RUN:
        ...
        print(f"   Found {len(messages)} message divs", flush=True)

        if not messages:
            break

        # CRITICAL FIX: Telegram returns messages from oldest to newest on each page.
        # Reverse so we process newest first.
        messages.reverse()
        ...
        for msg in messages:
            ...
            print(f"   Checking ID: {post_id}", flush=True)

            if last_known_id and post_id <= last_known_id:
                print(f"   🛑 {post_id} <= {last_known_id}, stopping.", flush=True)
                stop = True
                break
            ...
            if not text:
                print(f"   ⚠️ ID {post_id} has no text, skipping.", flush=True)
                continue
    ...
    # Return in ascending order (oldest first) for sending
    all_messages.sort(key=lambda x: x['id'])
    print(f"🏁 Returning {len(all_messages)} messages (IDs: {[m['id'] for m in all_messages]})", flush=True)
    return all_messages
...
def main():
    ...
    while time.time() - start_time < MAX_RUNTIME:
        ...
        if new_messages:
            print(f"   Sending in order: {[m['id'] for m in new_messages]}", flush=True)
